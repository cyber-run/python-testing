...
class DynaController:
    def __init__(self, com_port: str = '/dev/tty.usbserial-FT89FAA7', baud_rate: int = 115200) -> None:
    ...
    def set_pos(self, motor_id: int = 1, pos: float = 180) -> None:
        ...
        logging.info(f"Setting motor {motor_id} position to {pos} ticks")
        # Write to servo
    ...
    def set_sync_pos(self, pan_pos: float = 180, tilt_pos: float = 180) -> None:
        ...
        logging.info(f"Setting pan position to {pan_pos} ticks")
        logging.info(f"Setting tilt position to {tilt_pos} ticks")
    ...
    def write1ByteData(self, motor_id, address, value):
        ...
        if dxl_comm_result != COMM_SUCCESS:
            logging.info(self.packet_handler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
